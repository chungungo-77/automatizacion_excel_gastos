...
def normalizar_categoria(serie: pd.Series, catalogo: dict | None = None) -> tuple[pd.Series, int]:
    ...
    claves = serie.map(lambda x: _clave_normalizada(x) if pd.notna(x) else np.nan)
...
def detectar_filas_problematicas(df_original: pd.DataFrame) -> dict[str, pd.DataFrame]:
    """
    ...
    catálogo, fecha no parseable o monto no parseable.

    Se reutilizan las mismas funciones/catálogos que usa `limpiar_dataframe`,
    así que si mañana se agrega una variante nueva al catálogo, este panel
    de revisión queda consistente sin tocar nada más.
    ...
    if "Categoria" in df_original.columns:
        claves = df_original["Categoria"].map(
            lambda x: _clave_normalizada(x) if pd.notna(x) else np.nan
        )
        reconocida = claves.map(
            lambda k: (k in CATEGORIAS_CANONICAS) if pd.notna(k) else True
        )
    ...
    if "Fecha" in df_original.columns:
        fechas_parseadas = pd.to_datetime(
            df_original["Fecha"], dayfirst=True, errors="coerce", format="mixed"
        )
